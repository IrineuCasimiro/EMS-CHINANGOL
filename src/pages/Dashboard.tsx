...
import { EQUIPMENT_STATUS_LABELS, WORK_ORDER_STATUS_LABELS, ROLE_LABELS } from '@/lib/constants';
...
export function Dashboard({ onNavigate }: DashboardProps) {
  const { equipment = [], workOrders = [], requisitionItems = [] } = useData();
  ...
  const pendingReq = (requisitionItems || []).filter((r) => r.status === 'pending').length;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label="Active Work Orders" value={openWOCount} icon={<Wrench className="w-5 h-5" />} color="amber" trend={`${(workOrders || []).filter(w => w.status === 'pending_parts').length} pending parts`} />
        <StatCard label="Work Orders Total" value={(workOrders || []).length} icon={<ClipboardCheck className="w-5 h-5" />} color="teal" trend="Total registered" />
        <StatCard label="Pending Requisitions" value={pendingReq} icon={<Package className="w-5 h-5" />} color="red" trend={`${(requisitionItems || []).length} total items`} />
      </div>
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-muted-foreground truncate">{wo.client_project || 'No client specified'}</p>
                      </div>
                      <StatusBadge status={wo.status} label={WORK_ORDER_STATUS_LABELS[wo.status] || wo.status} />
                    </div>
                  );
                })
              )}
                ...
                criticalItems.map((eq) => (
                  <div key={eq.id} className="flex items-center justify-between p-3 rounded-md border border-border">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">{eq.location || 'N/A'} · {eq.serial_number || 'N/A'}</p>
                    </div>
                    <StatusBadge status={eq.status} label={EQUIPMENT_STATUS_LABELS[eq.status] || eq.status} />
                  </div>
                ))
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              <div className="p-3 rounded-md bg-muted/50">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">Pending Parts</span>
                </div>
                <p className="text-xl font-bold">{(workOrders || []).filter(w => w.status === 'pending_parts' || w.status === 'waiting_parts').length}</p>
              </div>
            </div>
            <div className="text-center py-2 text-xs text-muted-foreground">
              Chinangol LDA · Sany Department Management System
            </div>
          </CardContent>
        </Card>
      </div>
